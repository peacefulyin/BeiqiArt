...
export class MediaService {
  ...
  getVideoListById(id) {
    return this.http.get(`/api/videoList/${id}`);
  }

  getVideoListByName(name) {
    return this.http.get(`/api/videoList?search=${name}`);
  }

  getDefaultList() {
    return this.http.get(`/api/videoList/default`);
  }
  ...
  getIndexBanner() {
    return this.http.get('/api/banner?search=1').map(data => {
      // 因为search返回的list
      return data[0];
    });
  }

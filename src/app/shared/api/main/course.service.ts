...
export class CourseService {
  ...
  getCourseDetailById(id) {
    return this.http.get(`http://127.0.0.1/api/course/${id}/detail`);
  }

  getCourseDetailByName(name) {
    const nameOb = this.http.get(`http://127.0.0.1/api/course?search=${name}`).map(data => {
      return data[0].id;
    });
  ...
  getDefaultCourse() {
    return this.http.get(`http://127.0.0.1/api/course/default`);
  }

  getAllCourse() {
    return this.http.get(`http://127.0.0.1/api/course`);
  }
